...
namespace Conversa.Net.Xmpp.Xml
{
        ...
        : IDisposable
    {
        ...
        /// </summary>
        /// <returns></returns>
        internal StreamElement ReadNextNode()
        {
            ...
            while (!this.EOF && this.depth != 0)
            {
                ...
                if (next == '<' || this.currentTag.Length > 0)
                {
                    ...
                    if (!StreamParser.IsProcessingInstruction(tag))
                    {
                        if (this.node.Length == 0 && StreamParser.IsStreamStartTag(tag))
                        {
                            this.nodeName = StreamParser.GetTagName(tag);
                            this.depth    = 0;
                        }
                        else if (this.node.Length == 0 && StreamParser.IsStreamEndTag(tag))
                        {
                            this.nodeName = StreamParser.GetTagName(tag);
                            this.depth    = 0;
                        }
                        ...
                        {
                            if (!StreamParser.IsCharacterDataAndMarkup(tag))
                            {
                                if (StreamParser.IsStartTag(tag))
                                {
                                    if (this.depth == -1)
                                    {
                                        this.nodeName      = StreamParser.GetTagName(tag);
                                        this.nodeNamespace = StreamParser.GetXmlNamespace(tag);
            ...
            if (this.depth == 0)
            {
                result = new StreamElement(this.nodeName, this.nodeNamespace, this.node.ToString());
